...
function ArticleHeader({ id, counter }: { id: string; counter: string | number }) {
  return (
    <header className="relative mb-10 xl:mb-0">
      <div className="pointer-events-none absolute top-0 left-[max(-0.5rem,calc(50%-18.625rem))] z-50 flex h-4 items-center justify-end gap-x-2 lg:right-[calc(max(2rem,50%-38rem)+40rem)] lg:left-0 lg:min-w-[32rem] xl:h-8">
        <Link href={`#${id}`} className="inline-flex">
          <p className="hidden xl:pointer-events-auto xl:block xl:text-xs xl:font-medium xl:text-white/50">
            # {counter}
          </p>
        </Link>
      ...
      <ContentWrapper>
        <div className="flex">
          <Link href={`#${id}`} className="inline-flex">
            <p className="text-xs font-medium text-gray-500 xl:hidden dark:text-white/50">
              # {counter}
            </p>
          </Link>
        </div>
...
export const article = function Article({
  ...
  return (
    ...
    >
      <div ref={heightRef}>
        <ArticleHeader id={id} counter={counter} />
